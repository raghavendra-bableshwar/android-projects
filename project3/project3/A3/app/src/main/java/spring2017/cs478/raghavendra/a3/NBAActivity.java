...
public class NBAActivity extends AppCompatActivity implements NBATeamsFragment.ListSelectionListener {

    //Permission to be acquired
    private String mPermissionString = "edu.uic.cs478.project3";
    //Constant used in acquiring permission
    private final int NBA_PERMISSION_ID = 0;

    //Holds team names
    public static String[] mNbaTeams;

    //Holds team website URLs
    public static String[] mNbaTeamsUrls;

    //Teamlist fragment identifier
    public final String TEAMFRAGMENTTAG = "teamfragment";
    //Team website fragment identifier
    public final String TEAMWEBSITETAG = "teamwebsitetag";
    ...
    private final NBAWebsiteFragment mNbaWebsiteFragment = new NBAWebsiteFragment();
    ...
    * */
    void permissionGrantedActions(){
        ...
        mNbaTeams = getResources().getStringArray(R.array.nbateams);
        mNbaTeamsUrls = getResources().getStringArray(R.array.nbateamswebsites);
        mConfig = getResources().getConfiguration();

        // Get references to the TeamFragment and to the TeamWebsiteFragment
        mTeamsFrameLayout = (FrameLayout) findViewById(R.id.team_fragment_container);
        mTeamWebsiteFrameLayout = (FrameLayout) findViewById(R.id.websites_fragment_container);
        ...
        if(mFragmentManager.findFragmentByTag(TEAMFRAGMENTTAG) != null){
            ...
            if(mFragmentManager.findFragmentByTag(TEAMWEBSITETAG) != null){
                mFragmentTransaction.replace(R.id.websites_fragment_container,
                        mFragmentManager.findFragmentByTag(TEAMWEBSITETAG));
                Log.i(TAG,"Team website Found");
            }
            else {
                mFragmentTransaction.replace(R.id.team_fragment_container,
            ...
            mFragmentTransaction.replace(R.id.team_fragment_container,
                    new NBATeamsFragment(), TEAMFRAGMENTTAG);
    ...
    * */
    private void displayTeams(){
        ...
        if(!mNbaWebsiteFragment.isAdded()){
    ...
    * */
    ...
    public void onListSelection(int index) {
        ...
        if (!mNbaWebsiteFragment.isAdded()) {
            ...
            fragmentTransaction.replace(R.id.websites_fragment_container,
                    mNbaWebsiteFragment, TEAMWEBSITETAG);
        ...
        if (mNbaWebsiteFragment.getShownIndex() != index) {

            // Tell the Teamwebsiteview to show the website at position index
            mNbaWebsiteFragment.showQuoteAtIndex(index);
